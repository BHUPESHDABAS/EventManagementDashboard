...
router.post("/signup", async(req,res) =>{
    try {
        ...
        if(checkUser) return res.status(500).json({message: "User already exists, try another E-mail"});
...
router.post("/login", async(req,res) =>{
    try {
        ...
        if(password != checkUser.password) return res.status(500).json({message: "Incorrect password, try again"});
